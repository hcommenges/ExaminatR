...
<<univar1, echo=FALSE, results='markup', warning=FALSE, message=FALSE, cache=FALSE>>=
kable(UnivarDescription(df = readData(), varquanti = input$varuni))
@

<<univar2, echo=FALSE, results='markup', dev="my_png", fig.ext="png", warning=FALSE, message=FALSE, cache=FALSE>>=
for(i in 1:length(input$varuni)){
  print(Histogram(df = readData(), varquanti = input$varuni[i]))
}
@
