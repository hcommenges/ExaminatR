...
<<regress1, echo=FALSE, results='hide', dev="my_png", fig.ext="png", fig.pos="H", warning=FALSE, message=FALSE, cache=FALSE>>=
ScatterPlot3D(df = readData(), varx = input$varregress2[2], vary = input$varregress2[3], varz = input$varregress2[1])
@

~


<<regress2, echo=FALSE, results='markup', warning=FALSE, message=FALSE, cache=FALSE>>=
regResults2 <- ComputeRegression(df = readData(), 
